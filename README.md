...
- **データ出所**: 世界銀行 World Development Indicators (WDI)
  ...
  - `PA.NUS.FCRF` — 為替レート(円/ドル、期間平均)
  - `TT.PRI.MRCH.XD.WD` — 交易条件指数(2015年=100、2005年以降のみ公表)
  - `FP.CPI.TOTL.ZG` — 消費者物価上昇率(前年比%)
- **その他**: 厚生労働省「毎月勤労統計調査」長期時系列表(e-Stat経由) — 実質賃金指数(現金給与総額・事業所規模30人以上・2020年=100、1970年以降)
...
### 円安の影響の裏付け系列(為替・交易条件・物価・実質賃金)

![為替レート・交易条件・消費者物価・実質賃金の推移](japan_macro.png)
...
| `japan_macro.png` | 裏付け系列グラフ(為替レート・交易条件・消費者物価・実質賃金) |
| `japan_macro.csv` | 裏付け系列データ(年・為替・交易条件・物価・実質賃金) |
| `plot_japan_gnp.py` | ドル建てグラフの描画スクリプト |
| `plot_japan_gnp_yen.py` | 円建てグラフの描画スクリプト |
| `plot_japan_macro.py` | 裏付け系列グラフの描画スクリプト |
...
- 裏付け系列(為替レート・交易条件指数・消費者物価上昇率)を世界銀行WDIから取得し、グラフ・データ表をページとREADMEに追加
- 実質賃金指数(毎月勤労統計調査、e-Stat経由)を裏付け系列に追加。1997年ピーク(113.0)から2025年は98.0へ約13%低下、2023年は前年比-2.0%
